---
title: "BiFuzz: A Two-Stage Fuzzing Tool for Open-World Video Games"
authors: ["Yusaku Kato", "Norihiro Yoshida", "Erina Makihara", "Katsuro Inoue"]
venue: "情報処理学会 ソフトウェアエンジニアリングシンポジウム (SES) ポスター発表"
year: 2025
type: "conference"
tags: ["fuzzing", "software engineering", "video games"]
featured: false
---

本発表では、オープンワールドゲームにおけるキャラクターの移動障害（スタック等）を自動検出する2段階ファジングツール「BiFuzz」についてのポスター発表を行った。

This is a poster presentation of BiFuzz, a two-stage fuzzing tool for open-world video games, presented at SES 2025.
